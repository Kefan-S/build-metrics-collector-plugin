...
public class BuildInfoHandler implements BiConsumer<String, Run> {
    ...
    public void accept(String label, Run build) {
        buildDurationMetrics.labels(label).set(build.getDuration());
        buildResultMetrics.labels(label).set(build.getResult().ordinal);
        buildStartTimeMetrics.labels(label).set(build.getStartTimeInMillis());
